...
export default function WalletInfo() {
  ...
  const queryAddressBalance = async () => {
    const res = await fetch(`${API_URL}/clientBalance/${address}`, {
      method: 'GET',
      headers: {
  ...
  const queryAddressTxs = async () => {
    const res = await fetch(`${API_URL}/clientTransactions/${address}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
    ...
    result.txsInPool.reverse().forEach(tx => {
      const type = (tx.sender === address ? 'Send' : 'Receive');
    ...
    result.txs.reverse().forEach(tx => {
      const type = tx.sender === '' ? 'Reward mining' : (tx.sender === address ? 'Send' : 'Receive');
  ...
  return (
    <Container className={classes.container} >
      <Typography className={classes.container} component="p" color="primary" align="left" style={{ wordWrap: 'break-word' }} variant="h6">Address: {address}</Typography>
      <Grid container spacing={5} >
        <Grid item xs={12} md={7} >
          <Typography className={classes.container} color="primary" align="left" variant="h5">Transactions
            <IconButton onClick={() => { queryAddressTxs(); queryAddressBalance(); }} size='small'>
              <Refresh />
            </IconButton>
          </Typography>
        ...
        <Grid item xs={12} md={5}>
                    ...
                    <Table >
                      <TableBody style={{ marign: '10px' }}>
                        ...
                        <TableRow hover>
                          ...
                          <TableCell>
                            {txs[selectedTx].tx.sender === '' ? 'Coinbase txn' : (txs[selectedTx].tx.sender === address ? txs[selectedTx].tx.sender : <Link style={{ cursor: 'pointer' }} onClick={() => { history.push(`/explorer/address/${txs[selectedTx].tx.sender}`) }}>{txs[selectedTx].tx.sender}</Link>)}
                          </TableCell>
                        </TableRow>
                        <TableRow hover>
                          <TableCell style={{ width: '20%' }}> Receiver: </TableCell>
                          <TableCell>
                            {(txs[selectedTx].tx.receiver === address ? txs[selectedTx].tx.receiver : <Link style={{ cursor: 'pointer' }} onClick={() => { history.push(`/explorer/address/${txs[selectedTx].tx.receiver}`) }}>{txs[selectedTx].tx.receiver}</Link>)}
                          </TableCell>
                        </TableRow>
                        <TableRow hover>
          ...
          <Paper className={`${classes.smallPadding} ${classes.shadow}`} style={{ marginTop: '20px' }}>
            <Balance balance={balance} queryMyBalance={queryAddressBalance} />
          </Paper>
        </Grid>
